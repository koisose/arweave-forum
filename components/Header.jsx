    ...
    <>
      <div className="absolute top-[0px] right-[0px] py-10 z-50 w-[165px]">
        <div className="text-sm shadow-md bg-white border border-gray-200 p-3 rounded-md flex flex-col w-full space-y-1" ref={wrapperRef}>
          <div className="text-gray-900 font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2" onClick={() => setShowUserPopup(true)}>Update profile</div>
          <div className="text-gray-900 font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2" onClick={() => logout()}>Logout</div>
          ...
          {(search && search.length > 2) &&
            <div className="absolute top-[32px] right-[0px] z-50 w-[300px] ">
              <div className="text-sm shadow-md bg-white border border-gray-200 p-2 rounded-md flex flex-col w-full space-y-1">
                {loading ?
                  <div className="flex p-3 justify-center text-gray-900">
                    <LoadingCircle />
                  </div>
                :
                  <>
                      ...
                      <>
                        {posts.map((post, key) => {
                          if(post.content.master) {
                            return (
                              <Link key={post.stream_id} className="text-gray-500 font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2" href={"/post/" + post.content.master}>· {post.content.title ? post.content.title : <>{post.content.body.substring(0,25)}...</>}</Link>
                            );
                          } else {
                            return (
                              <Link key={post.stream_id} className="text-gray-900 font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2" href={"/post/" + post.stream_id}>{post.content.title ? post.content.title : <>{post.content.body.substring(0,25)}...</>}</Link>
                            );
                          }
          ...
          <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10 cursor-default" ref={wrapperRef}>
            <div className="pointer-events-auto w-screen" style={{width: 900}}>
              <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                <div className="bg-brand py-6 px-4 sm:px-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-3xl font-medium text-white deacon" id="slide-over-title">Community Chat</h2>
